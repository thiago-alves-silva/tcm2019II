...
<title>Deletar Atividade - System Crown</title>
</head>
<body>
    <div class="container" style="margin-top:5vw;">
		...
		<form name="frmContrato" method="post" action="excluirdemanda.php">
			<select name="sltContrato" disabled>					
				<<?php
			...
			<select name="sltConsultor" disabled>
				<?php 
			...
			<input type="text" name="txtTarefa" value="<?php echo $linha['nm_tarefa']; ?>" disabled> 
			<?php
				$i = $linha['dt_inicio'];
				$f = $linha['dt_final'];
			...
			?>
			<input type="text" name="txtDatainicial" value="<?php echo $dti; ?>" disabled>
			<input type="text" name="txtDataFinal" value="<?php echo $dtf; ?>" disabled>
			...
			<select name="sltPrioridade" disabled>
				<?php 
			...
			<textarea name="txtDescricao" disabled><?php echo $linha['ds_atividade']; ?></textarea>
			<textarea name="txtObs" disabled><?php echo $linha['ds_obs']; ?></textarea>            
